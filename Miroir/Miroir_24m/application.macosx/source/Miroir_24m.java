...
public class Miroir_24m extends PApplet {
...
//MIROIR alias SYPHON
public void beginMiroir() {
    miroir.beginDraw() ;
}

public void endMiroir(boolean display) {
  miroir.endDraw() ;
...
class EPSeaFly extends SuperCreature {
  ...
  public void draw(int colorIn, int colorOut, float thickness) {
    ...
    miroir.scale(0.2f);
    miroir.translate(count * 0.018f, count * 0.008f); 
    miroir.rotateX(count * 0.008f);
    ...
    miroir.rotateZ(aF/2 + 1.2f);
    miroir.rotateY(aF - 1);
    miroir.fill(hue(colorIn), saturation(colorIn), brightness(colorIn),alpha(colorIn)*.3f);
...
class MCManta extends SuperCreature {
  ...
  public void draw(int colorIn, int colorOut, float thickness) {
    ...
    for (int i=0; i<2; i++) {
      miroir.pushMatrix();
      miroir.translate(40 +i*15, 0 +(sin(i+count*0.1f)));
      miroir.scale(2, i*0.8f);
      miroir.sphere(15);
      miroir.popMatrix();
    }
    ...
    //AILE 1
    miroir.pushMatrix();
    miroir.rotateX(0.6f*sin(count * wingsAmp.x) + radians(90));
...
class PXPSonar extends SuperCreature {
  ...
  public void draw(int colorIn, int colorOut, float thickness) {
    ...
    miroir.rotateX(count*0.011f);
    miroir.rotateX(count*0.012f);
    miroir.rotateZ(count*0.013f);

    miroir.stroke (colorOut, alpha(colorOut) *.3f);    
    sphereDetail(6);
    miroir.sphere(30);    
    ...
    for (int k=0; k<numPinne; k++) {
      ...
      for (int j=0; j<numTentacoli; j++) {
        ...
        for (int i=0; i<numSegmenti; i++) {
          ...
          miroir.rotateY(b);
          miroir.rotateX(c);
          miroir.translate(l*0.9f, 0, 0);
          miroir.scale(0.85f, 0.85f, 0.85f);
          miroir.box(l, l/2, l); 
          //ellipse(l/2, l, l, l);
        }
...
class RomanescoThree extends SuperRomanesco {
  ...
  public void display() {
    ...
    for (Atom atm : atomList) {
      ...
      //GIVE the title from karaoke.txt
      String karaokeRaw [] = loadStrings(sketchPath("")+"karaoke.txt") ;
      //String karaokeRaw [] = loadStrings("text/text.txt") ;
      String karaoke = join(karaokeRaw, "") ;
      //tracking chapter
      String karaokeChapters [] = split(karaoke, "//////") ;
      //tracking sentence
      String karaokeSentences [] = split(karaokeChapters[0], "///") ;
      
      String word = karaokeSentences [0] ;
      //String word = "never release" ;
...
//:::::::Optional void atome ( position, detection...):::::::::::::::::::::::::::::::::::::::::
  public void drag2D() {
    //strokeWeight(d) ;
    insideA = radiusCursor2D() ;
    insideF = radiusElectronicFieldCursor2D() ;
    if(mousepressed[0] && insideA)   { lock = true; }
    if(!mousepressed[0])            { lock = false; }
    if(lock) 
    { 
      pos.x = mouseX; 
      pos.y = mouseY;
...
////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////COVALENT COLLISION//////////////////////////////////////////////////
  public void covalentCollision(ArrayList<Atom> listA ) // external
  {
    for (Atom target : listA) {
  ...
  public void contactCovalentEN(Atom target, PVector atomVect)  // internal
  {
  ...
  //::::::::::::
  public void statementCovalent(Atom target) // internal
  {
...
//:::::::::::::::::::::::the result depend of the positiv or negativ atom load.
  public void electronicCollision(ArrayList<Atom> listA, boolean ionicEffect ) // external
  {
    for (Atom target : listA) {
      if(ionicEffect) {
  ...
  public void contactElectronic(Atom target)  // internal
  {
...
//////////////////////RESOLVE COLLISION/////////////////////////////////
  public void resolveCollision(Atom target , PVector atomVect) // internal 
  {
...
//::::::::::::::::::::::::::ELECTRON::::::::::::::::::::::::::::::::::::::::::::::
  ...
  public void electronicInfo() // internal & external
  {
  ...
  public void display ( int c, float fs ) {
    if(alpha(c) != 0 ) {
      ...
      miroir.strokeWeight(thickness) ;
      if(insideA) { 
        miroir.fill(inAtom) ; miroir.stroke(inAtom) ;
      } else {
        miroir.fill(c) ; miroir.stroke(c) ;
        insideA = false ;
      }
      miroir.point (pos.x, pos.y) ;
    }
  }
  ...
  public void eCloudEllipse2D(int eColor, float amp, boolean cloud_, float thickness) {
    ...
    if ( alpha(eColor) != 0 ) {
      miroir.strokeWeight(thickness) ;
      miroir.stroke( eColor) ;
      miroir.ellipse (pos.x, pos.y, radiusElectronicFieldCovalent() *amp, radiusElectronicFieldCovalent() *amp ) ;
      miroir.ellipse (pos.x, pos.y, radiusElectronicField() *amp,     radiusElectronicField() *amp ) ;  
    }  
  }
////////////////DISPLAY///////////////////////////////////////////////////////////////////
//////////////Display text & Misc
  ...
  public void title2D(String title, int cName, PFont p, int sizeText, PVector posText ) {
    if ( alpha(cName) != 0 ) {
      miroir.fill(cName); 
      miroir.textFont(p, sizeText);
      miroir.textAlign(CENTER);
      miroir.text(title , pos.x +posText.x , pos.y +posText.y );
    }
  }
  //
  public void title2D(int cName, PFont p, int sizeText, PVector posText ) {
    if ( alpha(cName) != 0 ) {
      ...
      miroir.textFont(p, sizeText);
      miroir.textAlign(CENTER) ;
      miroir.text(nickName , pos.x +posText.x , pos.y +posText.y );
    }
  }
 ...
 /////////////////////DISPLAY PROPERTY of ATOM////////////////////////////////////////////
  public void titleAtom2D (int cName, int cInfo, PFont p, int sizeTextName, int sizeTextInfo, float amp_ ) {
    ...
    if ( alpha(cName) != 0 ) {
      miroir.fill(cName); miroir.textFont(p, sizeTextName);
      miroir.textAlign(CENTER);

      miroir.text(nameAtom[proton] , pos.x , pos.y + posYtext );
    }
    //Info
    if ( alpha(cInfo) != 0 ) {
      miroir.fill(cInfo); miroir.textFont(p, sizeTextInfo);
      miroir.textAlign (LEFT) ; 
      miroir.text(ion,              pos.x +posXtext , pos.y -posYtext );
      miroir.text(valenceElectron,  pos.x +posXtext , pos.y +( 2.3f *posYtext));
      //miroir.text(valenceElectron,  pos.x +posXtext , pos.y +sizeTextInfo);
      miroir.textAlign (RIGHT) ; 
     // miroir.text(proton,           pos.x -posXtext , pos.y +sizeTextInfo); 
      miroir.text(proton,           pos.x -posXtext , pos.y +( 2.3f *posYtext));
      miroir.text(round(mass),      pos.x -posXtext , pos.y -posYtext); 
      
      // capacity of atom for the Covalent Bond
      /*
      String capacity = c ;
      miroir.textFont(p, 10); textAlign(CENTER);
      miroir.text(capacity , pos.x , pos.y -15 ); 
      //Energy of atom
      String sE = nf(screenEffect, 1, 2 ) ;
      miroir.text("SE " + sE + " / EN " + electroNegativity , pos.x , pos.y +20 );
      */
    }
   
   //size of electronicCloud
   //miroir.text(radiusElectronicField() , pos.x , pos.y +20 ); 
...
//////////////////////////////////////RETURN///////////////////////////////////////////////////
  ...
  public boolean collide(Atom target, float targetRadius, float radius) 
  {
    ...
    if (distance < thresh) return true; else return false ;
  }
  ...
  {  
    ...
    if (distance > minThresh && distance < maxThresh) return true; else return false ;
  }
  //////RETURN MISC\\\\\\\\\\\\\\\\\\\\\
...
class Electron
{
  ...
  public void displayEllipse(float x_ , float y_ ,int d_ , int eColorFill, int eColorStroke, float eThick ) {
    ...
    if (alpha(eColorFill) != 0 && alpha( eColorStroke) != 0  ) {
      miroir.strokeWeight(thick) ;
      miroir.fill(eColorFill) ;
      miroir.stroke(eColorStroke) ;
      miroir.ellipse(x_, y_, d_, d_) ;
  ...
  public void display() {
   ...
   if (actionButton[IDobj] == 1 && nLongTouch && clickLongLeft[IDobj]) addEncre(factorPressure, sprayDirection, angleSpray, spray, inkDiffusion, inkDry, inkFlux) ; 
    ...
    for ( Encre e :  encreList ) {
      ...
      miroir.strokeWeight(thicknessPoint) ;
      miroir.noFill() ;
      miroir.stroke(c) ;
      miroir.point(e.x, e.y) ;
    }
...
class RomanescoSix extends SuperRomanesco 
{
  ...
  Arbre arbre ;
  // starting position
  PVector posArbre = new PVector (random(0, width), random(0,height) ) ;
  //CONSTRUCTOR
  ...
  //MOUSEPRESSED
  public void mousepressed() {
  //  if (spaceTouch) spaceTouchArbre = true ;
  }
  //MOUSE RELEASED
  public void mousereleased() {
   // spaceTouchArbre = false ;
  }
  //MOUSE DRAGGED
  public void mousedragged() {}
...
//::::::::::::::::::::::::::::  
  public void actualisation (PVector posArbre, int cIn, int cOut, int e, int nombre, int propA, int propB, int fourcheA, int fourcheB, float amplitude, int n, int mode, float angle)
  {
   ...
   // if ( rotation > 135  ) vR*=-1 ; else if ( rotation < 45 ) vR*=-1 ;
   ...
   if ( rotation > angle + 90  ) vR*=-1 ; else if ( rotation < angle ) vR*=-1 ; 
    ...
    pushMatrix () ;
    miroir.translate(posArbre.x,posArbre.y);
    // Start the recursive branching
    miroir.rotate (angleDirection) ;
...
//::::::::::::::::::::::::::::  
  ...
  public void displayBranch(int colorIn, int colorOut,float e, float proportion, int propA, int propB, int fourcheA, int fourcheB, float amplitude, int n, float t, int mode)
  {
    ...
    miroir.rotate(t);   // Rotate by theta

    miroir.strokeWeight (e ) ;
     if (mode == 0  ) miroir.line(0, 0, 0, -amplitude);  // Draw the branch
    if (mode == 1 ) miroir.ellipse(0,0, proportion  , proportion  ) ;
    if (mode == 2  ) {  miroir.ellipse(0,0, proportion  , proportion  ) ; miroir.line(0, 0, 0, -amplitude); }
    if (mode == 3 ) miroir.rect(0,0, proportion  , proportion  ) ;
    if (mode == 4  ) {  miroir.rect(0,0, proportion  , proportion  ) ; miroir.line(0, 0, 0, -amplitude); }
    miroir.translate(0, -amplitude); // Move to the end of the branch
...
class RomanescoNine extends SuperRomanesco 
{
 ...
 // ToxiclibsSupport gfx;
  ...
  public void display() {
    ...
    //.............
   // if (actionButton[IDobj] == 1 ) {
      miroir.translate(finalPos.x -posPoly.x, finalPos.y -posPoly.y) ;
      miroir.scale(ratioFinal) ;
    //} 
    ...
    for (Polygon2D p : polygons) {
      ...
      // to display or not
      if ( alphaIn == 0 ) miroir.noFill() ; else miroir.fill (colorIn) ;
      if ( alphaOut == 0) {
        miroir.noStroke() ;
      } else {
        miroir.stroke ( colorOut ) ; 
...
class RomanescoTwentyTwo extends SuperRomanesco 
{
  ...
  public void display() {
    ...
    //orientation / deg
    miroir.translate( mouse[IDobj].x, mouse[IDobj].y) ;
...
class Trame 
{
  ...
  {
    if( e < 0.1f ) e = 0.1f ; //security for the negative value
    miroir.stroke (cOut) ; miroir.strokeWeight(e) ;
  ...
  public void rectangleTrame( PVector pos, float h, float l, float gauche, float droite, float aObj) {
    ...
    miroir.translate(pos.x, pos.y) ;
    miroir.rotate(aObj) ;
    
    miroir.rect (0, 0, pow(l,1.4f), pow(h,1.4f)) ;
    miroir.rectMode(CORNER) ;
    popMatrix() ;
  }
  ...
  public void disqueTrame( PVector pos, float h, float l, float gauche, float droite, float aObj)
  {
    ...
    if (vd>360) vd =0 ;
    //display
    miroir.ellipseMode(CENTER) ;
    miroir.translate(pos.x, pos.y) ;
    miroir.rotate(aObj) ;
    miroir.ellipse (0, 0, pow(l,1.4f), pow(h,1.4f)) ;
    miroir.ellipseMode(CORNER) ;
    popMatrix() ;
  }
...
/////////////////////////////
    //CLEAR THE LIST IF NECESSARY 
    if (romanescoEmptyList(IDobj)) partRectList.clear() ;
...
{ 
  ...
  public void drawRect (int cIn, int cOut, float oIn, float oOut, float e, float h) {
    //security for the negative value
    if( e < 0.1f ) e = 0.1f ;
    ...
    miroir.strokeWeight(e) ;
    miroir.fill ( cIn, oIn ) ;
    miroir.stroke (cOut, oOut ) ;
    miroir.rect ( posX ,  -e, posX - ( mouse[0].x /3) , h+(2*e) ) ;
  }
}
...
class RomanescoTwentyFive extends SuperRomanesco 
{
  ...
  */
  ...
  int whichSizeCam = 4 ; // choice the resolution size of camera
    String whichCam = "1" ; // choice the camera
  ...
  public void display() {    
   ...
   //  factorDisplayPixel.y = factorDisplayCam.y / (20 -TD123/10 ) ; 
    ...
    if (testCam()) {
      ...
      for (int i = 0; i < cols ; i++) {
        for (int j = 0; j < rows  ; j++) {
         ...
         // color c = color(h, s, b);
          ...
          miroir.translate( ( (posPixelX +newDisplay.x *factorTranslateX) *factorDisplayCam.x) + posMouseCam.x - width *0.5f , 
                     ( (posPixelY +newDisplay.y *factorTranslateY) *factorDisplayCam.y) + posMouseCam.y - height*0.5f  );
                     
          miroir.rectMode(CENTER) ;
          miroir.fill(colorPixelCam) ;
          miroir.noStroke() ;
          miroir.rect (0,0, newCellSize.x, newCellSize.y) ;
          //DON'T TOUCH
          popMatrix() ;
          //END OF DON'T TOUCH
        }
      ...
      miroir.fill(0) ;
      testDeviceCam += 1 ;
      miroir.text("No extarnal video signal, Romanesco try on the native Camera", mouse[0].x , mouse[0].y ) ;
    } 
    ...
    if(!testCam() && testDeviceCam == 180 ) {
      miroir.fill(0) ;
      miroir.text("No camera available on your stuff my Friend !", mouse[0].x , mouse[0].y ) ;
    }
  ...
  */
  ...
  public void display() {
   ...
   // float vitesseSound = gaucheDroite ;
    ...
    /////////
    //UPDATE
    updateGrain(upTouch, downTouch, leftTouch, rightTouch, clickLongLeft[IDobj]);
  ...
  public void pointSable(float diam, int c) {
    for(int j = 0; j < grain.length; j++) {
     miroir.strokeWeight(diam) ;
     miroir.stroke(c) ;
      miroir.point(grain[j].x, grain[j].y);
  ...
  public void ellipseSable(PVector size, float e, int cIn, int cOut) {
    for(int j = 0; j < grain.length; j++) {
     miroir.strokeWeight(e) ;
      miroir.stroke(cOut) ;
      miroir.fill(cIn) ;
      miroir.ellipse(grain[j].x, grain[j].y, size.x, size.y);
      //set(int(grain[j].x), int(grain[j].y), colorIn);
    }
  }
  //
  public void rectSable(PVector size, float e, int cIn, int cOut) {
    for(int j = 0; j < grain.length; j++) {
      ...
      miroir.stroke(cOut) ;
      miroir.fill(cIn) ;
      miroir.rect(grain[j].x, grain[j].y, size.x, size.y);
      //set(int(grain[j].x), int(grain[j].y), colorIn);
    }
  }
  ...
  public void updateGrain(boolean up, boolean down, boolean left, boolean right, boolean mouseClic) {
    ...
    for(int i = 0; i < grain.length; i++) {
      ...
      if(grain[i].x < 0)     grain[i].x = width;
      if(grain[i].y > width) grain[i].y = 0;
      if(grain[i].y < 0)     grain[i].y = height;
    }
  }
  //END ANNEXE
...
class RomanescoFortyOne extends SuperRomanesco 
{
  ...
  public void display() {
    ...
    miroir.fill(colorIn); miroir.noStroke() ;
  ...
  public void displayLetter(int which, int c, float alpha, int ampJttr) {
    ...
    miroir.noFill() ; miroir.stroke(c) ; miroir.strokeWeight(alpha) ;
    for ( int i = 1; i < points.length; i++ ) {
      ...
      miroir.line( points[i-1].x, points[i-1].y, points[i-1].z,   points[i].x, points[i].y, points[i].z );
    }
  }
...
class RomanescoFortyTwo extends SuperRomanesco 
{
  ...
  public void display() {
    ...
    miroir.textFont(font[IDobj], corps + (mix[IDobj] *30));
    ...
    ///////////////
    //DON'T TOUCH
    pushMatrix() ;//P3D Give the position and the orientation of your object in the 3 dimensionals space
    P3Dmanipulation(IDobj) ;
    //END OF DON'T TOUCH
  ...
  public void horlogeCercle(PVector posHorloge, float angle, float  amp, int colorHorloge, int timeMode)
  {
    ...
    miroir.fill (colorHorloge) ;
    //seconde
    miroir.textAlign(CENTER, CENTER) ;
    miroir.translate(posHorloge.x, posHorloge.y) ;
    miroir.rotate(angle) ;
    miroir.text (nf(second(),2), cos(s)*amp*8 , sin(s)*amp*8 ) ;
    //minute
    miroir.text (nf(minute(),2), cos(m)*amp*6 , sin(m)*amp*6  ) ;
    //heure
    miroir.text(nf(hour()%timeMode ,2), cos(h)*amp*4 , sin(h)*amp*4  ) ;
    // text
    if ( timeMode == 12 ) if (hour() < 12 ) miroir.text("AM", 0, 0) ; else  miroir.text("PM", 0, 0 ) ; else miroir.text("TIME", 0, 0) ;
    
    miroir.textAlign(LEFT, TOP) ;
  }
  ...
  public void horlogeLigne(PVector posHorloge, float angle, float amp, int colorHorloge, int timeMode)
  {
    ...
    miroir.translate(posHorloge.x, posHorloge.y) ;
    miroir.rotate(angle) ;
    miroir.text( nf(hour()%timeMode,2)   + "." + 
          nf(minute(),2)   + "." + 
          nf(second(),2), 
          0, 0);
    miroir.textAlign(LEFT, TOP) ;
  ...
  public void horlogeMinute(PVector posHorloge, float angle, int colorHorloge ) 
  {
    ...
    miroir.translate(posHorloge.x, posHorloge.y) ;
    miroir.rotate(angle) ;
    miroir.text(hour() *60 + minute() + " MINUTES", 0,0) ;
    miroir.textAlign(LEFT, TOP) ;
  }
  ...
  public void horlogeSeconde(PVector posHorloge, float angle, int colorHorloge ) 
  {
    ...
    miroir.translate(posHorloge.x, posHorloge.y) ;
    miroir.rotate(angle) ;
    miroir.text((hour() *3600) + (minute() *60) + second() + " SECONDES", 0,0) ;
    miroir.textAlign(LEFT, TOP) ;
  }
...
class RomanescoFortyThree extends SuperRomanesco  {
  ...
  */
  ...
  public void display() {
    ...
    miroir.textFont(font[IDobj], corps + ( corps * mix[IDobj]) );
    // couleur du texte
    float t = valueObj[IDobj][4] * abs(mix[IDobj]) ;
    if ( soundButton[IDobj] != 1 ) { t = valueObj[IDobj][4] ; } 
    //color
    ...
    for( int i=info; i < info + 1; i++) {
      ...
      //rotation / degr\u00e9
      rotation(valueObj[IDobj][18], mouse[IDobj].x, mouse[IDobj].y) ;
      miroir.text(hune, 0, 0, largeur, hauteur );
    }
...
class FeedEntry {  
  ...
  public String toString() {  
    String s;
    String d = pubdate.toString();
    String date = d.substring(11) ;  
  
    s = title  ;
    //s = title + newline + date ;  
  /*
    if(description.length()>50)  
      s+="Contenu : "+description.substring(0,50)+  
        ".../..."+newline;  
    else s+="Contenu : "+description+"]"+newline;  
  ...
  */
  ...
  public void display() {
    ...
    if ( chapter < karaokeChapters.length) {
      ...
      rotation(valueObj[IDobj][18], mouse[IDobj].x, mouse[IDobj].y) ;
      //DISPLAY
      miroir.textAlign(CORNER);
      miroir.textFont(font[IDobj], corps + (mix[IDobj]) *6 *beat[IDobj]);
      miroir.fill(colorIn) ;
      miroir.text(karaokeSentences[sentence], 0 , 0 , l, h ) ;
    }
...
class RomanescoFortyFive extends SuperRomanesco {
  ...
  */
  ...
  public void display() {
    ...
    //DISPLAY
    miroir.textAlign(CORNER);
    miroir.textFont(font[IDobj], corps + ( corps * fsc) );
    ...
    if (clearList[IDobj] ) {
      ...
      yourList.clear() ;
      */
      //DON'T TOUCHE THIS LINE
      clearList[IDobj] = false ;
      //END OF DON4T TOUCH
    }
    //IT'S THE END FOR YOU
    //////////////////////
...
class MessageTwitter {
  ...
  {
    miroir.fill(c) ;
    miroir.text(mt, d, g, l, h) ;
       //  miroir.text(ttr, -(droite[45]*20) , -(gauche[45]*20) , l, h ) ;
  }
}
//////////////////
////////////////
...
class Twitter
{
  ...
  public void setup() {
    ...
    //println("Sc\u00e8ne " + cT[0]) ;
    //println("Sc\u00e8ne autorisation " + isOnline + " / " + twitterAcces) ;
...
class Pixel
{
  ...
  public void displayPixel(int diam, PVector effectColor)
  {
    ...
    miroir.stroke(hue(newColor), effectColor.x *saturation(newColor), effectColor.y *brightness(newColor), effectColor.z) ;
    if (newPos.x == 0 && newPos.y == 0 && newPos.z == 0) newPos = pos ; 
    miroir.point(newPos.x, newPos.y) ;
  }
...
public void backgroundP3D(int c) {
  ...
  miroir.translate(-sizeBG.x *.5f,-sizeBG.y *.5f , -sizeBG.z) ;
...
public void displayInfoScene() {
  ...
  //INFO MOUSE and PEN
  miroir.text("position X " + mouse[0].x + "  position Y " + mouse[0].y + "  molette " + wheel[0] + "    stylet orientation " + (int)deg360(pen[0]) +"\u00b0   stylet pression " + PApplet.parseInt(pen[0].z *10),15, 15 * (posInfoObj) ) ;  
  posInfoObj += 1 ;
  //INFO SOUND
...
public void P3Dmanipulation(int ID) {
  if(modeP3D) {
    ...
    miroir.translate(P3Dposition[ID].x, P3Dposition[ID].y, P3Dposition[ID].z) ;
    miroir.rotateX(radians(P3Ddirection[ID].x)) ;
    miroir.rotateY(radians(P3Ddirection[ID].y)) ;
  }
}
//starting position
...
public void startCamera(boolean scene, boolean eye, PVector speed) {
  ...
  //scene position
  miroir.translate(sceneCamera.x +width/2, sceneCamera.y +height/2, sceneCamera.z) ;
  //eye direction
  miroir.rotateX(radians(eyeCamera.x)) ;
  miroir.rotateY(radians(eyeCamera.y)) ;
...
/////////////////////
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Miroir_24m" };
    if (passedArgs != null) {
